...
namespace ChessGame.Logic
{
    public abstract class Move : IMove
    {
        ...
        {
            var eK = board.CurrentPlayerColor == Color.White ? Figure.BlackKing : Figure.WhiteKing;
        ...
        protected void HandleActionsCheckingKing(List<MoveOption> allMoveOptions, IBoard board)
        {
            ...
            {
                var nextMoveBoard = board.CurrentPlayerColor.Switch() == Color.White
                    ? new MoveWhiteFigure(new Board(board.Figures, board.CurrentPlayerColor.Switch(), new List<Log>() { action.Log })).InputBoard
                    : new MoveBlackFigure(new Board(board.Figures, board.CurrentPlayerColor.Switch(), new List<Log>() { action.Log })).InputBoard;
                ...
                {
                    if (nextMoveBoard.Figures.Where(x => x.Color == board.CurrentPlayerColor.Switch()).Any(x => x.MoveOptions.Any(x => _dangerousAction.Contains(x.Action))))
